...
void TestScene::LoadObject()
{
...

	{
		...
		Cube->SetLayerIndex(SceneManager::GetInstance()->LayerNameToIndex(L"Default"));
		...
		shared_ptr<Shader> CubeShader = ResourceManager::GetInstance()->Get<Shader>(L"Deferred");
...

	{
		...
		Cube2->SetLayerIndex(SceneManager::GetInstance()->LayerNameToIndex(L"Default"));
		...
		CubeTransform2->SetScale(XMFLOAT3(1.f, 1.f, 1.f));
	...
	//UI
	{
		...
		for (int i = 0; i < 3; i++)
		{
			...
			Cube2->SetCheckFrustum(false);
			shared_ptr<Transform>  CubeTransform2 = make_shared<Transform>();
			CubeTransform2->SetTranslate(XMFLOAT3(-330.f + (i * 160), 200.f, 0.f));
			CubeTransform2->SetRotateX(0.0f);
			CubeTransform2->SetRotateY(0.f);
			CubeTransform2->SetRotateZ(0.f);
			CubeTransform2->SetScale(XMFLOAT3(50.f, 50.f, 1.f));
			...
			shared_ptr<Texture>    CubeTexture2 = GEngine->GetRTGroup(RENDER_TARGET_GROUP_TYPE::G_BUFFER)->GetRTTexture(i);
			CubeMaretial2->SetTexture(CubeTexture2, 0);
		}
